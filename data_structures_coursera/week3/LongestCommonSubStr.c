...
HashSet precompute_hashvalues( char *str, int prime1,int prime2, int x )
{
	int len=strlen(str);
	HashSet h;
	long long *h1=malloc((len+1)*sizeof(long long));
	long long *h2=malloc((len+1)*sizeof(long long));
	h1[0]=0;
	h2[0]=0;
	for(int i=1;i<=len;++i){
...
{
	long long *p=malloc((len+1)*sizeof(long long));
	p[0]=1;
	for(int i=1;i<=len;++i)
		p[i]=x*p[i-1]%prime;
	return p;
...
Result common_substring_of_length_mid(HashSet hs,HashSet ht,int lenSubstr,int p1,int p2)
{
	...
	long long *substrhash_s=malloc(nSubstrh_s*sizeof(long long));
	for(int i=0;i<nSubstrh_s;++i)
		substrhash_s[i]=((hs.h1[i+lenSubstr]-p1*hs.h1[i])%prime1+prime1)%prime1;
	...
	for(int i=0;i<nSubstrh_t;++i)
	{
		...
		for(int j=0;j<nSubstrh_s;++j)
		{
			if(substrhash_t!=substrhash_s[j])
				continue;
...
int main()
{
	...
	t=malloc(100000*sizeof(char));
	while(scanf("%s%s",s,t)!=EOF)
	{
		Result r;
		r=longest_common_substring(s,t);//not resizing coz next string might be greater
		printf("%d %d %d\n",r.i,r.j,r.len);
	}
	free(s);
	free(t);
