...
Node* insert(Node *root, int data)
{
	...
	//if u don't want repeatition then search this node first, if present in the rbt then free(new_node); and then continue
...
void inOrder(Node *root)
{
	if(root == NULL)
		return;
	inOrder(root->left);
	printf("%d%c, ", root->data, ((root->color==0)?'r':'b'));
	inOrder(root->right);
}
...
int main()
{
	//DO NOT COMPILE THIS HERE OR RUN IT HERE, FOR SOME REASON THE SYSTEM DETECTS VIRUS
	Node *root = NULL;
	root = insert(root, 3);
	printf("Inorder : ");
	inOrder(root);
